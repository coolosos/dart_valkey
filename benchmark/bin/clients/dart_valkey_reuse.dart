...
Future<void> main(List<String> args) async {
  if (args.length < 2) {
    stderr.writeln(
      'Usage: dart run clients:dart_valkey_reuse <iterations> <warmup>',
    );
    exit(1);
  }
  ...
  // Create commands ONCE and reuse them
  final setCmd = SetCommand('reuse_key', 'reuse_value');
  final getCmd = GetCommand('reuse_key');
  final pingCmd = PingCommand();

  // Warmup
  for (var i = 0; i < warmupIterations; i++) {
    await client.execute(setCmd);
    await client.execute(getCmd);
    await client.execute(pingCmd);
  }

  // Benchmark - reusing same command objects
